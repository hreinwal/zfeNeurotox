...
# This script is designed to run DESeq2 normalization and statistical testing on RNAseq experiments
...
# will generate and export the annotated DESEq2 result tables from DGEA. These can be used for further
# downstream analysis such as GSEA or ORA. 

# Required Input: 
# - CountMatrix (txt or csv)
# - coldata (csv or tab; Sample annotation file in which row names must be identical to column names in count matrix)

# Main Analysis Steps are: 
# 1) Normalizing gene counts using DESeq2's negative binomial model
# 2) Run statistical modeling and testing (Pairwise Wald's t-test) against control with IHW for BH correction.
# 3) Visualize some basic statistics (pval distr., log2fc distr., dispersion model fit, etc. ...)
# 4) Annotated ENSEMBL gene IDs with gene_name, description, biotype, etc. using biomaRt
# 5) Filter for deferentially expressed genes (DEG)
# 5.1) Compute log2FC cut off (LFcut = quantile(abs(log2fc),0.9)) for each condition
# 5.2) shrink log2FC values via apeglm
# 5.3) Every gene with padj < 0.05 & abs(log2fc) > LFcut is considered as DEG
# 6) Export results to csv data tables for downstream analysis and data visualization.
##############
...
message("Biological replicae gene count correlation ...")
id.ls = list()
